...
class AFD():
    ...
    def minimize(self):
        # Step 1: Initialize a table with dimensions equal to the number of states
        table = [[False for _ in range(len(self.estados))] for _ in range(len(self.estados))]

        # Step 2: Mark pairs of states (p, q) if one is accepting and the other is not
        ...
        # Step 3: Mark pairs of states (p, q) if they have a transition to a marked pair of states
        change = True
        while change:
            change = False
            for i in range(len(self.estados)):
        ...
        # Step 4: Merge equivalent states and update the AFD
        groups = []
        for i in range(len(self.estados)):
            for j in range(i + 1, len(self.estados)):
                if not table[i][j]:
